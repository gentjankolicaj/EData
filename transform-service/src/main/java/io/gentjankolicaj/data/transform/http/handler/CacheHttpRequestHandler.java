...
import io.gentjankolicaj.data.transform.http.HttpConstants;
...
public class CacheHttpRequestHandler implements HttpRequestHandler {
    ...
    public void handle(ClassicHttpRequest request, ClassicHttpResponse response, HttpContext context) throws HttpException, IOException {
        if (request.getMethod().equalsIgnoreCase("GET")) {
            String cacheKey = request.getHeader(HttpConstants.CACHE_HEADER.getValue()).getValue();
            ...
            log.warn("HttpRequest not handled.");
        }
    }
}
